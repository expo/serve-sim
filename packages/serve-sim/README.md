...
To expose the preview to remote viewers behind a single port (the way standalone `serve-sim` does), pass `proxyHelpers: true`. The browser then reaches the stream, control socket, and DevTools through same-origin `/.sim/helper/<device>` and `/.sim/devtools` URLs. The control WebSocket is the exception (temporarily): it connects to the fixed `/.sim/helper/ws?device=<udid>` path (device in the query, not the path) so the WS endpoint stays the same across devices; the newer `/.sim/helper/<udid>/ws` form is also accepted. The per-device helper port and inspect-webkit bridge can stay local to the host. This routes WebSockets through the middleware, so you must forward your server's `upgrade` events to `handleUpgrade`:
