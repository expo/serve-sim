...
const pasteboardBuild = spawnSync(
  "bash",
  [
    resolve(root, "Sources/SimPasteboard/build.sh"),
    resolve(distDir, "simpb"),
  ],
  { stdio: "inherit" },
);
if (pasteboardBuild.status !== 0) {
  console.error("SimPasteboard build failed.");
  process.exit(pasteboardBuild.status ?? 1);
}
