...
describe("encoderLabel", () => {
  ...
  it("names the codec when the session is not on H.264", () => {
    // The bug this replaces: a VP8 session reported the H.264 probe and read as
    // "paravirt avc (hardware)" while the stream was software VP8.
  ...
  it("prefers the encoder id over the codec when they disagree", () => {
    expect(encoderLabel({
      id: "com.apple.videotoolbox.videoencoder.h264",
      hardware: false,
      codec: "VP8",
