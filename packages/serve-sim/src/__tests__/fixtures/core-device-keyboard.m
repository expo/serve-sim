...
static uintptr_t keySize = 2, stateSize = 1, optionalSize = 2;
static const char *scenario;
static unsigned sends, barriers, destroys, releases;
static uint8_t rawState;
static int errorObject;
static void *capability[5] = { NULL, NULL, NULL, &keySize, &stateSize };

static struct MetadataResponse __attribute__((swiftcall)) keyType(uintptr_t request) {
    return (struct MetadataResponse){ .metadata = &keySize };
}
static struct MetadataResponse __attribute__((swiftcall)) stateType(uintptr_t request) {
    return (struct MetadataResponse){ .metadata = &stateSize };
}
static struct MetadataResponse __attribute__((swiftcall)) optionalType(uintptr_t request, void *metadata) {
    assert(metadata == &stateSize);
    return (struct MetadataResponse){ .metadata = &optionalSize };
}
