 ...
 * timeout: if AVCC produces no decoded H.264 frame within the window, we fall
 * back to MJPEG (which every helper serves) and stay there for the session. A
 * JPEG seed may paint immediately, but it does not prove the encoder works.
 *
...
export interface AvccFallbackState {
  /** True once AVCC has yielded a decoded H.264 frame for the current stream. */
  streamed: boolean;
...
export type AvccFallbackEvent =
  /** A decoded H.264 frame was painted under AVCC. */
  | "decoded-frame"
...
export function avccFallbackReducer(
  ...
  switch (event) {
    ...
    case "timeout":
      // Only fall back if AVCC never decoded a frame. A later stall (helper
 ...
 * enough for a healthy encoder to emit its first decoded frame, short enough
 * that a dead AVCC endpoint does not strand the preview on a static JPEG seed.
 */
