...
export function useSenderStats(
  ...
  const [stats, setStats] = useState<SenderView>({ session: null, capture: null, encoder: null, stale: false });
  const lastSampleAt = useRef(0);

  useEffect(() => {
    // Reset unconditionally so a device switch drops the previous device's samples.
    lastSampleAt.current = 0;
    setStats({ session: null, capture: null, encoder: null, stale: false });
    ...
    const sample = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          if (!stopped) setStats({ session: null, capture: null, encoder: null, stale: false });
          return;
        }
